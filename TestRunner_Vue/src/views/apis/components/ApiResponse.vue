            ...
            <div class="tw-p-4">
              <div v-if="response.data?.response?.content" class="tw-bg-gray-900/50 tw-rounded-lg tw-shadow-inner tw-relative group">
                <div
                  ...
                  @click="copyContent(JSON.stringify(response.data.response.content, null, 2))"
                  title="复制"
                >
                  <icon-copy />
                </div>
                <pre class="tw-p-4 tw-text-gray-300 tw-font-mono tw-text-sm tw-leading-6 tw-whitespace-pre-wrap tw-break-all">{{ JSON.stringify(response.data.response.content, null, 2) }}</pre>
              </div>
          ...
          <div class="tw-h-full tw-overflow-auto">
            <div class="tw-p-4">
              <div v-if="response.data?.response?.headers" class="tw-bg-gray-900/50 tw-rounded-lg tw-shadow-inner tw-relative group">
                <div
                  class="tw-absolute tw-right-2 tw-top-2 tw-cursor-pointer copy-button"
                  @click="copyContent(JSON.stringify(response.data.response.headers, null, 2))"
                  title="复制"
                >
                  <icon-copy />
                </div>
                <pre class="tw-p-4 tw-text-gray-300 tw-font-mono tw-text-sm tw-leading-6 tw-whitespace-pre-wrap tw-break-all">{{ JSON.stringify(response.data.response.headers, null, 2) }}</pre>
              </div>
          ...
          <div class="tw-h-full tw-overflow-auto">
            <div class="tw-p-4">
              <div v-if="response.data?.request" class="tw-bg-gray-900/50 tw-rounded-lg tw-shadow-inner tw-relative group">
                <div
                  class="tw-absolute tw-right-2 tw-top-2 tw-cursor-pointer copy-button"
                  @click="copyContent(JSON.stringify(response.data.request, null, 2))"
                  title="复制"
                >
                  <icon-copy />
                </div>
                <pre class="tw-p-4 tw-text-gray-300 tw-font-mono tw-text-sm tw-leading-6 tw-whitespace-pre-wrap tw-break-all">{{ JSON.stringify(response.data.request, null, 2) }}</pre>
              </div>
                ...
                >
                  ...
                  <div class="tw-flex tw-items-center tw-justify-between tw-mb-2">
                    <div class="tw-flex tw-items-center tw-gap-2">
                      ...
                      <span class="tw-text-gray-300 tw-font-medium">{{ result.comparator }}</span>
                    </div>
                  </div>
          ...
          <div class="tw-h-full tw-overflow-auto">
            <div class="tw-p-4">
              <div v-if="response" class="tw-bg-gray-900/50 tw-rounded-lg tw-shadow-inner tw-relative group">
                <div
                  class="tw-absolute tw-right-2 tw-top-2 tw-cursor-pointer copy-button"
                  @click="copyContent(JSON.stringify(response, null, 2))"
                  title="复制"
                >
                  <icon-copy />
                </div>
                <pre class="tw-p-4 tw-text-gray-300 tw-font-mono tw-text-sm tw-leading-6 tw-whitespace-pre-wrap tw-break-all">{{ JSON.stringify(response, null, 2) }}</pre>
          </div>
          <a-empty v-else description="暂无响应数据" />
            </div>
        </div>
...
/* 新的复制按钮样式 */
.copy-button {
  @apply tw-flex tw-items-center tw-justify-center tw-w-8 tw-h-8 tw-bg-gray-800 tw-rounded hover:tw-bg-gray-700;
